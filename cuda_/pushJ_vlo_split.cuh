...
pushJ_vlo_reduce_Jtmp(double Jtmp, double *Jtmp_shd, long __idx) {
...
pushJ_vlo_deposit_Jx12(double rx1x, double dix1x, double rx2x, double dix2x,
  ...
  for ((xyzz = 0); (xyzz < 5); (xyzz = (xyzz + 1))) {
    ...
    for ((xyzy = 0); (xyzy < 5); (xyzy = (xyzy + 1))) {
      ...
      for ((xyzx = 0); (xyzx < 5); (xyzx = (xyzx + 1))) {
        ...
        double Jtmp = (simd_mask * (((if1_x2)[xyzx] - (if1_x1)[xyzx]) *
...
static __device__ __noinline__ void
pushJ_vlo_deposit_Jy12(double rx1y, double dix1y, double rx2y, double dix2y,
  ...
  for ((xyzz = 0); (xyzz < 5); (xyzz = (xyzz + 1))) {
    ...
    for ((xyzy = 0); (xyzy < 5); (xyzy = (xyzy + 1))) {
      ...
      for ((xyzx = 0); (xyzx < 5); (xyzx = (xyzx + 1))) {
        int allidx = (xyzx + (5 * (xyzy + (5 * xyzz))));
        double Jtmp = (simd_mask * (((if1_y2)[xyzy] - (if1_y1)[xyzy]) *
...
static __device__ __noinline__ void
pushJ_vlo_deposit_Jz13(double rx1z, double dix1z, double rx3z, double dix3z,
  ...
  for ((xyzz = 0); (xyzz < 5); (xyzz = (xyzz + 1))) {
    ...
    for ((xyzy = 0); (xyzy < 5); (xyzy = (xyzy + 1))) {
      ...
      for ((xyzx = 0); (xyzx < 5); (xyzx = (xyzx + 1))) {
        int allidx = (xyzx + (5 * (xyzy + (5 * xyzz))));
        double Jtmp = (simd_mask * (((if1_z3)[xyzz] - (if1_z1)[xyzz]) *
...
static __device__ __noinline__ void
pushJ_vlo_deposit_Jy23(double rx2y, double dix2y, double rx3y, double dix3y,
  ...
  for ((xyzz = 0); (xyzz < 5); (xyzz = (xyzz + 1))) {
    ...
    for ((xyzy = 0); (xyzy < 5); (xyzy = (xyzy + 1))) {
      ...
      for ((xyzx = 0); (xyzx < 5); (xyzx = (xyzx + 1))) {
        int allidx = (xyzx + (5 * (xyzy + (5 * xyzz))));
        double Jtmp = (simd_mask * (((if1_y3)[xyzy] - (if1_y2)[xyzy]) *
...
static __device__ __noinline__ void
pushJ_vlo_deposit_Jx23(double rx2x, double dix2x, double rx3x, double dix3x,
  ...
  for ((xyzz = 0); (xyzz < 5); (xyzz = (xyzz + 1))) {
    ...
    for ((xyzy = 0); (xyzy < 5); (xyzy = (xyzy + 1))) {
      ...
      for ((xyzx = 0); (xyzx < 5); (xyzx = (xyzx + 1))) {
        int allidx = (xyzx + (5 * (xyzy + (5 * xyzz))));
        double Jtmp = (simd_mask * (((if1_x3)[xyzx] - (if1_x2)[xyzx]) *
...
static __device__ __noinline__ void
pushJ_vlo_K2_push_x2(pushJ_vlo_state_t *st, double x0, double x1,
                     double allincfvx, double DELTA_X, double Deltat,
...
static __device__ __noinline__ void
pushJ_vlo_K2_push_y2(pushJ_vlo_state_t *st, double x0, double y0, double y1,
...
static __device__ __noinline__ void
pushJ_vlo_K2_push_z2_z3(pushJ_vlo_state_t *st, double z0, double z1, double iBz,
...
static __device__ __noinline__ void
pushJ_vlo_K2_push_y3(pushJ_vlo_state_t *st, double y1, double iBy, double TX0,
...
static __device__ __noinline__ void
pushJ_vlo_K2_push_x3(pushJ_vlo_state_t *st, double x1, double y1, double iBx,
...
static __device__ __noinline__ double
pushJ_vlo_K2_compute_iBy(const pushJ_vlo_state_t *st, const double *shB0,
                         const double *shB1) {
  double iBy_term0 = pushJ_B_integralS_comp2_f0z_dif1x_f1y(
      shB0, st->s_rx1z, st->s_dix1z, st->s_rx1x, st->s_dix1x, st->s_rx0x,
      st->s_dix0x, st->s_rx1y, st->s_dix1y);
  double iBy_term1 = pushJ_B_integralS_comp2_f0z_dif1x_f1y(
      shB1, st->s_rx1z, st->s_dix1z, st->s_rx2x, st->s_dix2x, st->s_rx1x,
      st->s_dix1x, st->s_rx1y, st->s_dix1y);
  return (iBy_term0 + iBy_term1);
}

static __device__ __noinline__ double
pushJ_vlo_K2_compute_iBz(const pushJ_vlo_state_t *st, const double *shB0,
                         const double *shB1) {
  double iBz_term1 = pushJ_B_integralS_comp0_f0x_dif1y_f1z(
      shB0, st->s_rx0x, st->s_dix0x, st->s_rx1y, st->s_dix1y, st->s_rx0y,
      st->s_dix0y, st->s_rx1z, st->s_dix1z);
  double iBz_term2 = pushJ_B_integralS_comp1_f0y_dif1x_f1z(
      shB0, st->s_rx1y, st->s_dix1y, st->s_rx1x, st->s_dix1x, st->s_rx0x,
      st->s_dix0x, st->s_rx1z, st->s_dix1z);
  double iBz_term3 = pushJ_B_integralS_comp0_f0x_dif1y_f1z(
      shB1, st->s_rx2x, st->s_dix2x, st->s_rx2y, st->s_dix2y, st->s_rx1y,
      st->s_dix1y, st->s_rx1z, st->s_dix1z);
  double iBz_term4 = pushJ_B_integralS_comp1_f0y_dif1x_f1z(
      shB1, st->s_rx1y, st->s_dix1y, st->s_rx2x, st->s_dix2x, st->s_rx1x,
      st->s_dix1x, st->s_rx1z, st->s_dix1z);
  double inner = ((-1.00000000000000000e+00 * iBz_term1) + iBz_term2 +
                  (-1.00000000000000000e+00 * iBz_term3) + iBz_term4);
  return (-1.00000000000000000e+00 * inner);
}

static __device__ __noinline__ double
pushJ_vlo_K2_compute_iBy23(const pushJ_vlo_state_t *st, const double *shB1) {
  return ((-1.00000000000000000e+00 *
           pushJ_B_integralS_comp0_f0x_dif1z_f1y(
               shB1, st->s_rx2x, st->s_dix2x, st->s_rx2z, st->s_dix2z,
               st->s_rx1z, st->s_dix1z, st->s_rx2y, st->s_dix2y)) +
          (-1.00000000000000000e+00 *
           pushJ_B_integralS_comp0_f0x_dif1z_f1y(
               shB1, st->s_rx2x, st->s_dix2x, st->s_rx3z, st->s_dix3z,
               st->s_rx2z, st->s_dix2z, st->s_rx2y, st->s_dix2y)));
}

static __device__ __noinline__ double
pushJ_vlo_K2_compute_iBx(const pushJ_vlo_state_t *st, const double *shB1) {
  return (pushJ_B_integralS_comp1_f0y_dif1z_f1x(
              shB1, st->s_rx2y, st->s_dix2y, st->s_rx2z, st->s_dix2z,
              st->s_rx1z, st->s_dix1z, st->s_rx2x, st->s_dix2x) +
          pushJ_B_integralS_comp1_f0y_dif1z_f1x(
              shB1, st->s_rx2y, st->s_dix2y, st->s_rx3z, st->s_dix3z,
              st->s_rx2z, st->s_dix2z, st->s_rx2x, st->s_dix2x) +
          (-1.00000000000000000e+00) *
              pushJ_B_integralS_comp2_f0z_dif1y_f1x(
                  shB1, st->s_rx1z, st->s_dix1z, st->s_rx2y, st->s_dix2y,
                  st->s_rx1y, st->s_dix1y, st->s_rx2x, st->s_dix2x) +
          (-1.00000000000000000e+00) *
              pushJ_B_integralS_comp2_f0z_dif1y_f1x(
                  shB1, st->s_rx3z, st->s_dix3z, st->s_rx3y, st->s_dix3y,
...
static __device__ __noinline__ void
pushJ_vlo_K2_stage1a_E(const pushJ_vlo_state_t *st, const double *shE0,
                       double Deltat, double *allincfvx_out,
                       double *allincfvy_out, double *allincfvz_out) {
  const double AllEx =
      pushJ_E_interp_Ex(shE0, st->s_rx1x, st->s_dix1x, st->s_rx1y, st->s_dix1y,
                        st->s_rx1z, st->s_dix1z);
  const double AllEy =
      pushJ_E_interp_Ey(shE0, st->s_rx1x, st->s_dix1x, st->s_rx1y, st->s_dix1y,
                        st->s_rx1z, st->s_dix1z);
  const double AllEz =
      pushJ_E_interp_Ez(shE0, st->s_rx1x, st->s_dix1x, st->s_rx1y, st->s_dix1y,
                        st->s_rx1z, st->s_dix1z);
...
static __device__ __noinline__ void pushJ_vlo_K2_stage1b_x2y2(
    pushJ_vlo_state_t *st, const double *shB0, const double *shB1, double x0,
    double x1, double y0, double y1, double allincfvx, double allincfvy,
    double DELTA_X, double DELTA_Y, double Deltat, double TX0, double QE_MASS,
    double XO, double YO, double Midx, double Midy) {
...
static __device__ __noinline__ void
pushJ_vlo_K2_stage2_z2z3(pushJ_vlo_state_t *st, const double *shB0,
                         const double *shB1, double z0, double z1,
                         double allincfvz, double DELTA_Z, double Deltat,
                         double QE_MASS, double ZO, double zmid, double Midz) {
...
static __device__ __noinline__ void
pushJ_vlo_K2_stage3a_y3(pushJ_vlo_state_t *st, const double *shB1, double y1,
                        double TX0, double DELTA_Y, double Deltat,
...
static __device__ __noinline__ void
pushJ_vlo_K2_stage3b_x3(pushJ_vlo_state_t *st, const double *shB1, double x1,
...
static __device__ __noinline__ void pushJ_vlo_K2_stage1_E_x2y2(
    pushJ_vlo_state_t *st, const double *shE0, const double *shB0,
    const double *shB1, double x0, double x1, double y0, double y1,
    double DELTA_X, double DELTA_Y, double Deltat, double TX0, double QE_MASS,
    double XO, double YO, double Midx, double Midy, double *allincfvz_out) {
  double allincfvx, allincfvy;
  pushJ_vlo_K2_stage1a_E(st, shE0, Deltat, &allincfvx, &allincfvy,
                         allincfvz_out);
  pushJ_vlo_K2_stage1b_x2y2(st, shB0, shB1, x0, x1, y0, y1, allincfvx,
                            allincfvy, DELTA_X, DELTA_Y, Deltat, TX0, QE_MASS,
                            XO, YO, Midx, Midy);
}

static __device__ __noinline__ void
pushJ_vlo_K2_stage3_y3x3(pushJ_vlo_state_t *st, const double *shB1, double x1,
                         double y1, double TX0, double DELTA_X, double DELTA_Y,
                         double Deltat, double QE_MASS, double XO, double YO,
                         double Midx, double Midy) {
  pushJ_vlo_K2_stage3a_y3(st, shB1, y1, TX0, DELTA_Y, Deltat, QE_MASS, YO,
                          Midy);
  pushJ_vlo_K2_stage3b_x3(st, shB1, x1, y1, TX0, DELTA_X, DELTA_Y, Deltat,
                          QE_MASS, XO, Midx);
}

static __device__ __noinline__ void pushJ_vlo_K2_push_weights_x2x3(
    pushJ_vlo_state_t *st, const double *shE0, const double *shB0,
  ...
  double allincfvz;
  pushJ_vlo_K2_stage1_E_x2y2(st, shE0, shB0, shB1, x0, x1, y0, y1, DELTA_X,
                             DELTA_Y, Deltat, TX0, QE_MASS, XO, YO, Midx, Midy,
                             &allincfvz);
  pushJ_vlo_K2_stage2_z2z3(st, shB0, shB1, z0, z1, allincfvz, DELTA_Z, Deltat,
                           QE_MASS, ZO, zmid, Midz);
  pushJ_vlo_K2_stage3_y3x3(st, shB1, x1, y1, TX0, DELTA_X, DELTA_Y, Deltat,
...
static __device__ __noinline__ void pushJ_vlo_K3_deposit_writeback_particle(
    ...
    int bBidy, int bBidz) {
  pushJ_vlo_deposit_Jx12(st->s_rx1x, st->s_dix1x, st->s_rx2x, st->s_dix2x,
                         st->s_rx1y, st->s_dix1y, st->s_rx1z, st->s_dix1z,
                         st->simd_mask,
                         st->_gen_long_for_opt_loop_already_idx_0, __idx, shJ0,
                         Jtmp_shd, grid_geo_x, Charge0, Tori_X0, XO, bBidx);
  pushJ_vlo_deposit_Jy12(st->s_rx1y, st->s_dix1y, st->s_rx2y, st->s_dix2y,
                         st->s_rx2x, st->s_dix2x, st->s_rx1z, st->s_dix1z,
                         st->simd_mask,
                         st->_gen_long_for_opt_loop_already_idx_0, __idx, shJ0,
                         Jtmp_shd, grid_geo_y, Charge0, Tori_X0, XO, bBidx);
  pushJ_vlo_deposit_Jz13(st->s_rx1z, st->s_dix1z, st->s_rx3z, st->s_dix3z,
                         st->s_rx2x, st->s_dix2x, st->s_rx2y, st->s_dix2y,
                         st->simd_mask,
                         st->_gen_long_for_opt_loop_already_idx_0, __idx, shJ0,
                         Jtmp_shd, grid_geo_z, Charge0, Tori_X0, XO, bBidx);
  pushJ_vlo_deposit_Jy23(st->s_rx2y, st->s_dix2y, st->s_rx3y, st->s_dix3y,
                         st->s_rx2x, st->s_dix2x, st->s_rx3z, st->s_dix3z,
                         st->simd_mask,
                         st->_gen_long_for_opt_loop_already_idx_0, __idx, shJ0,
                         Jtmp_shd, grid_geo_y, Charge0, Tori_X0, XO, bBidx);
  pushJ_vlo_deposit_Jx23(st->s_rx2x, st->s_dix2x, st->s_rx3x, st->s_dix3x,
