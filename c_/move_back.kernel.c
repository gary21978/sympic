...
int c_is_in(long x, long xl, long xr) { return ((x < xr) && (x >= xl)); }
void c_move_back_kernel_6_scmc_kernel(double *inoutput, int *xyzw,
  ...
  for (i = 0; (i < cu_load); (i = (i + 64))) {
...

    {
      ...
      for (0; (MYGEN5 < (numcp * 6)); (MYGEN5 = (MYGEN5 + __xlen))) {
    ...
    if ((__idx == 0)) {
      for (j = 0; (j < numcp); j++) {
        ...
        double dxyzx = ((local_temp_particle_cache + (j * 6)))[0];

        double dxyzy = ((local_temp_particle_cache + (j * 6)))[1];

        double dxyzz = ((local_temp_particle_cache + (j * 6)))[2];

        int wrted_grid = 0;

        if ((c_is_in(xyzx, 0, XLEN) &&
             (c_is_in(xyzy, 0, YLEN) && c_is_in(xyzz, 0, ZLEN)))) {
          long allid_grid =
...
void c_move_back_kernel_8_scmc_kernel(double *inoutput, int *xyzw,
  ...
  for (i = 0; (i < cu_load); (i = (i + 64))) {
...

    {
      ...
      for (0; (MYGEN6 < (numcp * 8)); (MYGEN6 = (MYGEN6 + __xlen))) {
    ...
    if ((__idx == 0)) {
      for (j = 0; (j < numcp); j++) {
        ...
        double dxyzx = ((local_temp_particle_cache + (j * 8)))[0];

        double dxyzy = ((local_temp_particle_cache + (j * 8)))[1];

        double dxyzz = ((local_temp_particle_cache + (j * 8)))[2];
