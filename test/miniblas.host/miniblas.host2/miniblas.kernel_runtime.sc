#include "miniblas.kernel_runtime.h"
#include "miniblas.kernel.h"
#ifdef __cplusplus
extern "C" {
#endif

void printy(mapu_pscmc_mem* y, char* funcName)
{
    mapsMemcpy(y->h_data, y->d_data, y->len * y->type_len, mapsMemcpyDeviceToHost);
    double *hy = (double*)(y->h_data);
    for (MAPU_LONG j = 0; j < y->len; j++)
    {
        printf("%s y[%lld]: %.16f\n", funcName, j, hy[j]);
    }

    return;
    static int times = 0;
    ++times;
    char fname[64];
    snprintf(fname, 64, "%s_%d", funcName, times);
    FILE *fp = fopen(fname, "wb");
    fwrite(y->h_data, y->type_len, y->len, fp);
    fclose(fp);
}
int mapu_blas_axpy_Field3D_Seq(Field3D_Seq *pthis, Field3D_Seq *y, double a, Field3D_Seq *x)
{
    char* dx = ((mapu_pscmc_mem*)(x->main_data))->d_data;
    char* dy = ((mapu_pscmc_mem*)(y->main_data))->d_data;
    mapu_blas_axpy<<<pthis->numvec, 1>>>((__DDR double *)dy, (__DDR double *)dx, a, y->numvec, y->numvec, y->xlen, y->ylen, y->zlen, y->ovlp, y->xblock, y->yblock, y->zblock, y->num_ele);
    printy((mapu_pscmc_mem*)(y->main_data), "axpy");
    return 0;
}

int mapu_blas_yisax_Field3D_Seq(Field3D_Seq *pthis, Field3D_Seq *y, double a, Field3D_Seq *x)
{
    char* dx = ((mapu_pscmc_mem*)(x->main_data))->d_data;
    char* dy = ((mapu_pscmc_mem*)(y->main_data))->d_data;
    mapu_blas_yisax<<<pthis->numvec, 1>>>((__DDR double *)dy, (__DDR double *)dx, a, y->numvec, y->numvec, y->xlen, y->ylen, y->zlen, y->ovlp, y->xblock, y->yblock, y->zblock, y->num_ele);
    printy((mapu_pscmc_mem*)(y->main_data), "yisax");
    return 0;
}

int mapu_blas_yisax_enlarge_Field3D_Seq(Field3D_Seq *pthis, Field3D_Seq *y, double a, Field3D_Seq *x)
{
    char* dx = ((mapu_pscmc_mem*)(x->main_data))->d_data;
    char* dy = ((mapu_pscmc_mem*)(y->main_data))->d_data;
    mapu_blas_yisax_enlarge<<<pthis->numvec, 1>>>((__DDR double *)dy, (__DDR double *)dx, a, y->numvec, y->numvec, y->xlen, y->ylen, y->zlen, y->ovlp, y->xblock, y->yblock, y->zblock, y->num_ele);
    printy((mapu_pscmc_mem*)(y->main_data), "yisax_enlarge");
    return 0;
}

int mapu_blas_yiszero_Field3D_Seq(Field3D_Seq *pthis, Field3D_Seq *y)
{
    char* dy = ((mapu_pscmc_mem*)(y->main_data))->d_data;
    mapu_blas_yiszero<<<pthis->numvec, 1>>>((__DDR double *)dy, y->numvec, y->numvec, y->xlen, y->ylen, y->zlen, y->ovlp, y->xblock, y->yblock, y->zblock, y->num_ele);
    printy((mapu_pscmc_mem*)(y->main_data), "yiszero");
    return 0;
}

int mapu_blas_yiszero_synced_Field3D_Seq(Field3D_Seq *pthis, Field3D_Seq *y)
{
    char* dy = ((mapu_pscmc_mem*)(y->main_data))->d_data;
    mapu_blas_yiszero_synced<<<pthis->numvec, 1>>>((__DDR double *)dy, y->numvec, y->numvec, y->xlen, y->ylen, y->zlen, y->ovlp, y->xblock, y->yblock, y->zblock, y->num_ele);
    printy((mapu_pscmc_mem*)(y->main_data), "yiszero_synced");
    return 0;
}

#ifdef __cplusplus
} /* extern "C" */
#endif