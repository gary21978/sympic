...
void c_move_back_kernel_8_get_struct_len(size_t *len) {
  ((len)[0] = sizeof(c_move_back_kernel_8_struct));
}
int c_move_back_kernel_8_get_xlen() { return IDX_OPT_MAX; }
...
int c_move_back_kernel_8_exec(c_move_back_kernel_8_struct *kerstr,
                              ...
                              long scmc_internal_g_ylen) {
  {
    ...
    for ((yid = ymin); ((yid < ymax) && (yid < scmc_internal_g_ylen));
         (yid = (yid + 1))) {
      c_move_back_kernel_8_scmc_kernel(
          (kerstr)->inoutput, (kerstr)->xyzw, (kerstr)->cu_cache,
          (kerstr)->cu_xyzw, ((kerstr)->XLEN)[0], ((kerstr)->YLEN)[0],
          ((kerstr)->ZLEN)[0], ((kerstr)->grid_cache_len)[0],
...
void c_move_back_kernel_6_get_struct_len(size_t *len) {
  ((len)[0] = sizeof(c_move_back_kernel_6_struct));
}
int c_move_back_kernel_6_get_xlen() { return IDX_OPT_MAX; }
...
int c_move_back_kernel_6_exec(c_move_back_kernel_6_struct *kerstr,
                              ...
                              long scmc_internal_g_ylen) {
  {
    ...
    for ((yid = ymin); ((yid < ymax) && (yid < scmc_internal_g_ylen));
         (yid = (yid + 1))) {
      c_move_back_kernel_6_scmc_kernel(
          (kerstr)->inoutput, (kerstr)->xyzw, (kerstr)->cu_cache,
          (kerstr)->cu_xyzw, ((kerstr)->XLEN)[0], ((kerstr)->YLEN)[0],
          ((kerstr)->ZLEN)[0], ((kerstr)->grid_cache_len)[0],
          ((kerstr)->cu_cache_length)[0], yid, scmc_internal_g_ylen);
