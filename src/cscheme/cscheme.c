		...
		*/
		...
		switch(ele->type){
			...
			case EMPTYLIST:
				...
				break;
			case PAIR:
				fprintf(stderr,"pair: car=%p cdr=%p\n",car(ele),cdr(ele));
				break;
			case ELSE:
...
void write_element(element * ele,FILE *out){
	...
	switch (ele->type){
		...
		case MACRO:
			...
			break;
		case INPUT_PORT:
			fprintf(out,"#<input-port:0x%p>",ele->data.in_port.in_port);
			break;
		case OUTPUT_PORT:
			fprintf(out,"#<output-port:0x%p>",ele->data.out_port.out_port);
			break;
		case ELSE:
			fprintf(out,"#<else>");
			break;
...
element * internal_read(element * input){
	...
	if(in_port != stdin && car(input)->data.out_port.isclose){
		fprintf(stderr,"Error in read: input-port 0x%p closed\n",car(input)->data.out_port.out_port);
		ERR_REPORT(1);
		//exit(1);
	}
...
element * internal_read_char(element * input){
	FILE * in_port= input==empty_list_process ? stdin:car(input)->data.out_port.out_port;
	if(in_port != stdin && car(input)->data.out_port.isclose){
		fprintf(stderr,"Error in read-char: input-port 0x%p closed\n",car(input)->data.out_port.out_port);
		ERR_REPORT(1);
		//exit(1);
	}
	input=alloc_element();
...
element * internal_peek_char(element * input){
	FILE * in_port= input==empty_list_process ? stdin:car(input)->data.out_port.out_port;
	if(in_port != stdin && car(input)->data.out_port.isclose){
		fprintf(stderr,"Error in peek-char: input-port 0x%p closed\n",car(input)->data.out_port.out_port);
		ERR_REPORT(1);
	}
	input=alloc_element();
	input->type=MCHAR;
...
element * internal_write(element * input){
	...
	if(out_port!=stdout && car(input)->data.in_port.isclose){
		fprintf(stderr,"Error in write: output-port 0x%p closed\n",car(input)->data.in_port.in_port);
		ERR_REPORT(1);
		//exit(1);
	}
...
element * internal_write_string(element * input){
	...
	FILE * out_port= (input==empty_list_process) ? stdout:car(input)->data.in_port.in_port;
	if(out_port!=stdout && car(input)->data.in_port.isclose){
		fprintf(stderr,"Error in write-string: output-port 0x%p closed\n",car(input)->data.in_port.in_port);
		ERR_REPORT(1);
	}
	//write_element(etw,out_port);
	if(etw->type!=STRING){
