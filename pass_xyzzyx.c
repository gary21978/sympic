...
int split_pass_xyz_zyx_mpi_shell(Particle_in_Cell_MPI *pthis, double dt0,
                                 int xyzzyx, int merge_J) {
  ...
  if ((use_small_grid == 1)) {
    ...
    if ((use_small_grid == 2)) {
      blas_yiszero_synced_Field3D_MPI(&(pthis->MPI_LFoutJ),
                                      &(pthis->MPI_LFoutJ));
  ...
  if ((USE_FILTER == 3)) {
    blas_yisax_Field3D_MPI(&(pthis->MPI_fieldEtmp1), &(pthis->MPI_fieldEtmp1),
                           1, &(pthis->MPI_fieldE));
    blas_yisax_Field3D_MPI(&(pthis->MPI_fieldBtmp1), &(pthis->MPI_fieldBtmp1),
                           1, &(pthis->MPI_fieldB));
  ...
  if ((USE_FILTER == 1)) {
    blas_mulxy_Field3D_MPI(&(pthis->MPI_fieldE), &(pthis->MPI_fieldE),
                           &(pthis->MPI_fieldE_filter));
    sync_ovlp_mpi_field(&(pthis->MPI_fieldE));
    blas_mulxy_Field3D_MPI(&(pthis->MPI_fieldB), &(pthis->MPI_fieldB),
                           &(pthis->MPI_fieldB_filter));
  ...
  if ((USE_FILTER == 3)) {
    blas_axpy_Field3D_MPI(&(pthis->MPI_fieldEtmp1), &(pthis->MPI_fieldEtmp1),
                          -1, &(pthis->MPI_fieldE));
    blas_axpy_Field3D_MPI(&(pthis->MPI_fieldBtmp1), &(pthis->MPI_fieldBtmp1),
                          -1, &(pthis->MPI_fieldB));
  ...
  if (USE_INIT_EXT_EB) {
    blas_axpy_Field3D_MPI(&(MPI_fieldB), &(MPI_fieldB), 1.00000000000000000e+00,
                          &(MPI_fieldB_ext));
  ...
  if (xyzzyx) {
    if (1) {
      ...
      if ((use_small_grid && merge_J)) {
        ...
        init_parallel_file_for_mpi_fields(&(pthis->MPI_LFoutJ), pgid, "tmpJdbg",
                                          -1, 0, 0);
  ...
  if ((USE_FILTER == 3)) {
    blas_axpy_Field3D_MPI(&(pthis->MPI_fieldE), &(pthis->MPI_fieldE), 1,
                          &(pthis->MPI_fieldEtmp1));
    blas_axpy_Field3D_MPI(&(pthis->MPI_fieldB), &(pthis->MPI_fieldB), 1,
                          &(pthis->MPI_fieldBtmp1));
