...
int build_Field_compute_unit_from_seq_field(Field3D_Seq *pthis,
                                            Field3D_Seq *input_E) {
  ...
  int num_cu =
      (((0 == (input_E)->CD_type))
           ? (c_blas_sum_get_num_compute_units(NULL))
           : ((((1 == (input_E)->CD_type))
                   ? (openmp_blas_sum_get_num_compute_units(NULL))
                   : ((((2 == (input_E)->CD_type))
                           ? (cuda_blas_sum_get_num_compute_units(NULL))
                           : (0))))));
...
int build_Field_compute_unit_from_mpi_field(Field3D_MPI *pthis,
                                            Field3D_MPI *input_E) {
  ...
  ((pthis)->data = malloc((sizeof(Field3D_Seq) * (input_E)->num_runtime)));
  {
    ...
    for ((i = 0); (i < (input_E)->num_runtime); (i = (i + 1))) {
      build_Field_compute_unit_from_seq_field((pthis->data + i),
                                              (input_E->data + i));
    }
  }
}
