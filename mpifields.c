...
int cuda_alloc_Field3D_Seq(Field3D_Seq *pthis, int is_init_kernels) {
  ...
  if (is_init_kernels) {
    ...
    cuda_sync_ovlp_o2m_all_in_one_init(pe, ((pthis)->sync_kernels)[11]);
    {
      ...
      cuda_kgm_eqn_core_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[0] = malloc(structlen));
    }
    cuda_kgm_eqn_core_init(pe, ((pthis)->fdtd_kernels)[0]);
    {
      size_t structlen;

      cuda_kgm_calc_rho_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[1] = malloc(structlen));
    }
    cuda_kgm_calc_rho_init(pe, ((pthis)->fdtd_kernels)[1]);
    {
      size_t structlen;

      cuda_PML_FDTD_CURL_BWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[2] = malloc(structlen));
    }
    cuda_PML_FDTD_CURL_BWD_init(pe, ((pthis)->fdtd_kernels)[2]);
    {
      size_t structlen;

      cuda_PML_FDTD_CURL_FWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[3] = malloc(structlen));
    }
    cuda_PML_FDTD_CURL_FWD_init(pe, ((pthis)->fdtd_kernels)[3]);
    {
      size_t structlen;

      cuda_merge_current_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[4] = malloc(structlen));
    }
    cuda_merge_current_init(pe, ((pthis)->fdtd_kernels)[4]);
    {
      size_t structlen;

      cuda_merge_current_2_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[5] = malloc(structlen));
    }
    cuda_merge_current_2_init(pe, ((pthis)->fdtd_kernels)[5]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Div_FWD_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[6] = malloc(structlen));
    }
    cuda_Yee_FDTD_Div_FWD_4th_init(pe, ((pthis)->fdtd_kernels)[6]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Div_FWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[7] = malloc(structlen));
    }
    cuda_Yee_FDTD_Div_FWD_init(pe, ((pthis)->fdtd_kernels)[7]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Div_BWD_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[8] = malloc(structlen));
    }
    cuda_Yee_FDTD_Div_BWD_4th_init(pe, ((pthis)->fdtd_kernels)[8]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Div_BWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[9] = malloc(structlen));
    }
    cuda_Yee_FDTD_Div_BWD_init(pe, ((pthis)->fdtd_kernels)[9]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_FWD_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[10] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_FWD_4th_init(pe, ((pthis)->fdtd_kernels)[10]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_FWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[11] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_FWD_init(pe, ((pthis)->fdtd_kernels)[11]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_BWD_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[12] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_BWD_4th_init(pe, ((pthis)->fdtd_kernels)[12]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_BWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[13] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_BWD_init(pe, ((pthis)->fdtd_kernels)[13]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Grad_FWD_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[14] = malloc(structlen));
    }
    cuda_Yee_FDTD_Grad_FWD_4th_init(pe, ((pthis)->fdtd_kernels)[14]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Grad_FWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[15] = malloc(structlen));
    }
    cuda_Yee_FDTD_Grad_FWD_init(pe, ((pthis)->fdtd_kernels)[15]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Grad_BWD_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[16] = malloc(structlen));
    }
    cuda_Yee_FDTD_Grad_BWD_4th_init(pe, ((pthis)->fdtd_kernels)[16]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Grad_BWD_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[17] = malloc(structlen));
    }
    cuda_Yee_FDTD_Grad_BWD_init(pe, ((pthis)->fdtd_kernels)[17]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_B_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[18] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_B_4th_init(pe, ((pthis)->fdtd_kernels)[18]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_B_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[19] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_B_init(pe, ((pthis)->fdtd_kernels)[19]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_E_4th_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[20] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_E_4th_init(pe, ((pthis)->fdtd_kernels)[20]);
    {
      size_t structlen;

      cuda_Yee_FDTD_Curl_E_get_struct_len(&(structlen));
      (((pthis)->fdtd_kernels)[21] = malloc(structlen));
    }
    cuda_Yee_FDTD_Curl_E_init(pe, ((pthis)->fdtd_kernels)[21]);
