...
#if SCHEME_DEBUG
#define ERR_REPORT(errornum)                                                                                           \
  {                                                                                                                    \
    long i = 0;                                                                                                        \
    while (global_caller != empty_list_process && i < 100) {                                                           \
      fprintf(stderr, "\n#%ld ", i);                                                                                   \
      write_element(car(global_caller)->data.gc_stack.tmpele[0], stderr);                                              \
...
void cal_dependency(element *ele) {
  if (ele == NULL)
    return;
  if (((allocated_list *)(ele->p_al))->using)
    return;
...
int issymbol_char(int c) {
  return isalpha(c) || c == '+' || c == '-' || c == '*' || c == '/' || c == '>' || c == '<' || c == '_' || c == '=' ||
         c == '$' || c == '%' || c == '\\' || c == '&' || c == '^' || c == '!' || c == '?' || c == '@' || c > 127;
}
...
int is_self_eval(element *ele) {
  return ele->type == INTNUM || ele->type == MFLOATNUM || ele->type == MCHAR || ele->type == MBOOLEAN ||
         ele->type == STRING;
}
...
#define SWITCHVECTOR(ele, intp, floatp, generalp)                                                                      \
  switch (ele) {                                                                                                       \
  case INTVEC:                                                                                                         \
    intp;                                                                                                              \
    break;                                                                                                             \
...
element *internal_vec_ref(element *ele) {
  ...
  SWITCHVECTOR(self->data.vector.type, return makeint(self->data.vector.vector_data.p_int[k]),
               return makefloat(self->data.vector.vector_data.p_float[k]),
               return self->data.vector.vector_data.p_general[k]);
...
element *internal_vec_set(element *ele) {
  ...
  SWITCHVECTOR(
      self->data.vector.type, if (val->type != INTNUM) { internal_vec_expand_int(self); },
      if (val->type != MFLOATNUM) { internal_vec_expand_int(self); }, break);
  SWITCHVECTOR(self->data.vector.type, self->data.vector.vector_data.p_int[k] = val->data.intnum,
               self->data.vector.vector_data.p_float[k] = val->data.floatnum,
               self->data.vector.vector_data.p_general[k] = val);
  return done_symbol;
}
...
element *expand_cond(element *input) {
  if (isempty(input)) {
    ...
    if (is_tag(car(input), else_symbol)) {
      ...
      return cons(if_symbol, cons(car(car(input)), cons(cons(begin_symbol, cdr(car(input))),
                                                        cons(expand_cond(cdr(input)), empty_list_process))));
    }
  }
}
...
tailcall_opt:
  ...
  if (is_tag(input, if_symbol)) {
    if (cdr(input) == empty_list_process || cdr(cdr(input)) == empty_list_process ||
        cdr(cdr(cdr(input))) == empty_list_process) {
  ...
  if (is_tag(input, lambda_symbol)) {
    ret = make_lambda_proc(car(cdr(input)), cdr(cdr(input)), env);
    goto ret_mark;
  }
  ...
  if (input->type == SYMBOL) {
    ret = find_var_in_env(input, env, 0);
    goto ret_mark;
  }
  write_element(input, stderr);
  fprintf(stderr, "\nunknown expression\n");
...
void init_environment(element *global_environment) {
  ...
  define_var_in_env(makesymbol("internal-make-single-env-from-var-and-val"), makeproc(make_single_env_from_var_and_val),
                    global_environment);
  define_var_in_env(makesymbol("add-binding-in-single-env"), makeproc(add_binding_in_single_env), global_environment);
  define_var_in_env(makesymbol("internal-find-var-in-single-env"), makeproc(internal_find_var_in_single_env),
                    global_environment);
