        ...
        if ((type == PS_DOUBLE_NUM)) {
          ...
          if ((type == PS_SHORT_NUM)) {
            ...
            if ((type == PS_CHAR)) {
              ...
              fprintf(stderr,
                      "Error: unknown type in init_pscmc_mem: type=%d\n", type);
            }
          }
        }
      }
...
int c_pscmc_flush(c_pscmc_env *pe) { return 0; }
...
int c_pscmc_get_local_len() { return 1; }
int c_pscmc_mem_destroy(c_pscmc_mem *pm) {
  free(pm->d_data);
  return 0;
}
int c_pscmc_get_num_compute_units(c_pscmc_env *pe) { return 1; }
int c_pscmc_mem_sync_d2h(c_pscmc_mem *pm) { return 0; }
int c_pscmc_mem_sync_h2d(c_pscmc_mem *pm) { return 0; }
int c_pscmc_env_init(c_pscmc_env *pe, int dev_num, void *fv) { return 0; }
size_t c_pscmc_mem_get_num(c_pscmc_mem *pm) { return pm->len; }
