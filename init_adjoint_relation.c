...
int init_complexity_tid(unsigned long num_proc, unsigned long num_thread,
                        size_t *ori_vec, long *num_thread_array,
                        int64_t *local_tid_array, int *cd_performances,
                        int num_cds_per_mpi_proc) {
...

  {
    ...
    for ((i = 0); (i < num_thread); (i = (i + 1))) {
      ((cplx_array)[i] = ((fp) ? ((cplx_ori_array)[(ori_vec)[i]])
                               : (floor(call_CAL_FUN_ONE_PARA(
                                     "get_thread_complexity", (ori_vec)[i])))));
  ...
  ((num_thread_array)[0] = 0);
  {
    ...
    for ((i = 0); (i < num_thread); (i = (i + 1))) {
      ...
      if (((global_cplx * ((num_proc / num_cds_per_mpi_proc) * total_perf)) >=
           (current_perf_all * total_complexity))) {
...
int init_uniform_tid(unsigned long num_proc, unsigned long num_thread,
                     long *num_thread_array, long *local_tid_array,
      ...
      for ((k = 0); (k < num_cds_per_mpi_proc); (k = (k + 1))) {
        ...
        double cur_proc_num_threads_f =
            ((1.00000000000000000e+00 *
              ((cd_performances)[k] * num_t_one_mpi_proc)) /
             total_perf);

        long cur_proc_num_threads =
            floor((cur_proc_num_threads_f +
                   ((1 - (1.00000000000000000e+00 / num_cds_per_mpi_proc)) +
...
long get_cur_tid(long pid, long num_thread, long num_proc,
                 long *num_thread_array) {
  return (num_thread_array)[pid];
}
long get_cur_num_tid(long pid, long num_thread, long num_proc,
                     long *num_thread_array) {
  long ret = 0;

  assert((pid < num_proc));
  (ret = (((pid < (num_proc - 1)))
              ? (((num_thread_array)[(pid + 1)] - (num_thread_array)[pid]))
              : ((num_thread - (num_thread_array)[pid]))));
  return ret;
}
long find_the_proc_id(long tid, unsigned long num_proc,
                      long *num_thread_array) {
  if ((num_proc <= 1)) {
    return 0;
    ...
    if ((tid < (num_thread_array)[(num_proc / 2)])) {
      ...
      return ((num_proc / 2) +
              find_the_proc_id(tid, (num_proc - (num_proc / 2)),
                               (num_thread_array + (num_proc / 2))));
    }
  }
}
int init_adjoint_relations(Field3D_Seq *pthis, long n, long ndim, long adj_type,
                           long num_proc, long *ovvec, long *ori_vec,
                           long *num_thread_array, long *local_tid_array) {
  ...
  void *blas_invy_full_block_kernel = (pthis)->blas_invy_full_block_kernel;

  void *blas_axpby_kernel = (pthis)->blas_axpby_kernel;

  void *blas_axpby_full_block_kernel = (pthis)->blas_axpby_full_block_kernel;

  void *blas_axpy_kernel = (pthis)->blas_axpy_kernel;

  void *blas_axpy_full_block_kernel = (pthis)->blas_axpy_full_block_kernel;

  void *blas_yisax_kernel = (pthis)->blas_yisax_kernel;

  void *blas_yisax_full_block_kernel = (pthis)->blas_yisax_full_block_kernel;

  void *blas_mulxy_kernel = (pthis)->blas_mulxy_kernel;

  void *blas_mulxy_full_block_kernel = (pthis)->blas_mulxy_full_block_kernel;
  ...
  void *blas_dot_full_block_kernel = (pthis)->blas_dot_full_block_kernel;

  void *blas_sum_kernel = (pthis)->blas_sum_kernel;

  void *blas_sum_full_block_kernel = (pthis)->blas_sum_full_block_kernel;

  void *blas_yisax_shrink_kernel = (pthis)->blas_yisax_shrink_kernel;

  void *blas_yisax_enlarge_kernel = (pthis)->blas_yisax_enlarge_kernel;

  void *blas_axpy_shrink_kernel = (pthis)->blas_axpy_shrink_kernel;

  void *blas_axpy_enlarge_kernel = (pthis)->blas_axpy_enlarge_kernel;

  void *blas_axpby_shrink_kernel = (pthis)->blas_axpby_shrink_kernel;

  void *blas_axpby_enlarge_kernel = (pthis)->blas_axpby_enlarge_kernel;
  ...
  for (i = 0; (i < numvec); (i = (i + 1))) {
    ...
    ((global_z_offset)[i] = (zlen * bxyzz));
    {
      int *tmpv =
          (((0 == CD_type))
               ? (c_pscmc_get_h_data(cur_rankx_pscmc, NULL))
               : ((((1 == CD_type))
                       ? (openmp_pscmc_get_h_data(cur_rankx_pscmc, NULL))
                       : ((((2 == CD_type))
                               ? (cuda_pscmc_get_h_data(cur_rankx_pscmc, NULL))
                               : (0))))));

      ((tmpv)[i] = ((2 * (bxyzx == (x_num_thread_block - 1))) + (bxyzx == 0)));
      (tmpv = (((0 == CD_type))
                   ? (c_pscmc_get_h_data(xoffset, NULL))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_get_h_data(xoffset, NULL))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_get_h_data(xoffset, NULL))
                                   : (0)))))));
    ...
    }
    {
      int *tmpv =
          (((0 == CD_type))
               ? (c_pscmc_get_h_data(cur_ranky_pscmc, NULL))
               : ((((1 == CD_type))
                       ? (openmp_pscmc_get_h_data(cur_ranky_pscmc, NULL))
                       : ((((2 == CD_type))
                               ? (cuda_pscmc_get_h_data(cur_ranky_pscmc, NULL))
                               : (0))))));

      ((tmpv)[i] = ((2 * (bxyzy == (y_num_thread_block - 1))) + (bxyzy == 0)));
      (tmpv = (((0 == CD_type))
                   ? (c_pscmc_get_h_data(yoffset, NULL))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_get_h_data(yoffset, NULL))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_get_h_data(yoffset, NULL))
                                   : (0)))))));
    ...
    }
    {
      int *tmpv =
          (((0 == CD_type))
               ? (c_pscmc_get_h_data(cur_rankz_pscmc, NULL))
               : ((((1 == CD_type))
                       ? (openmp_pscmc_get_h_data(cur_rankz_pscmc, NULL))
                       : ((((2 == CD_type))
                               ? (cuda_pscmc_get_h_data(cur_rankz_pscmc, NULL))
                               : (0))))));

      ((tmpv)[i] = ((2 * (bxyzz == (z_num_thread_block - 1))) + (bxyzz == 0)));
      (tmpv = (((0 == CD_type))
                   ? (c_pscmc_get_h_data(zoffset, NULL))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_get_h_data(zoffset, NULL))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_get_h_data(zoffset, NULL))
                                   : (0)))))));
          ...
          for ((xyzy = 0); (xyzy < 3); (xyzy = (xyzy + 1))) {
            {
              ...
              for ((xyzx = 0); (xyzx < 3); (xyzx = (xyzx + 1))) {
                ...
                long adj_id_pre =
                    (0 + (1 * ((((bxyzx + (xyzx - 1)) + x_num_thread_block) %
                                x_num_thread_block) +
                               (x_num_thread_block *
                                ((((bxyzy + (xyzy - 1)) + y_num_thread_block) %
                                  y_num_thread_block) +
                                 (y_num_thread_block *
                                  (((bxyzz + (xyzz - 1)) + z_num_thread_block) %
                                   z_num_thread_block)))))));
                ...
                long cur_adj_pid =
                    find_the_proc_id(cur_adj_id, num_proc, num_thread_array);

                ((adj_processes)[((i * NUM_SYNC_LAYER) + xyzlocalall)] =
                     cur_adj_pid);
                ((adj_local_tid)[((i * NUM_SYNC_LAYER) + xyzlocalall)] =
                     (local_tid_array)[cur_adj_id]);
              }
            }
          }
        }
