...
size_t fwrite_multi_proc(char *ptr, size_t size, size_t nmemb, FILE *stream,
                         int cur_proc, int num_one_group, int num_max_proc) {
  size_t ret = 0;
  ...
  if ((io_proc == cur_proc)) {
    (ret = (ret + fwrite(ptr, size, nmemb, stream)));
    {
      ...
      for ((g = (io_proc + 1)); (g < (io_proc + num_one_group));
           (g = (g + 1))) {
        if ((g >= num_max_proc)) {
          break;
        ...
        for (0; (l < alllen); (l = (l + 16777216))) {
          ...
          PS_MPI_Recv(IO_BLOCK_BUFFER, num_send, PS_MPI_CHAR, g, (g * 2),
                      PS_MPI_COMM_WORLD, &(pms));
    ...
    PS_MPI_Send(&(alllen), 1, PS_MPI_LONG, io_proc, cur_proc,
                PS_MPI_COMM_WORLD);
    long l = 0;
    ...
    for (0; (l < alllen); (l = (l + 16777216))) {
      ...
      PS_MPI_Send((ptr + l), num_send, PS_MPI_CHAR, io_proc, (cur_proc * 2),
                  PS_MPI_COMM_WORLD);
    }
  }
...
size_t fread_multi_proc(char *ptr, size_t size, size_t nmemb, FILE *stream,
                        int cur_proc, int num_one_group, int num_max_proc) {
  size_t ret = 0;
  ...
  if ((io_proc == cur_proc)) {
    (ret = (ret + fread(ptr, size, nmemb, stream)));
    {
      ...
      for ((g = (io_proc + 1)); (g < (io_proc + num_one_group));
        ...
        for (0; (l < alllen); (l = (l + 16777216))) {
          ...
          PS_MPI_Send(IO_BLOCK_BUFFER, num_send, PS_MPI_CHAR, g, (g * 2),
                      PS_MPI_COMM_WORLD);
        }
      }
    }
    ...
    for (0; (l < alllen); (l = (l + 16777216))) {
      ...
      PS_MPI_Recv((ptr + l), num_send, PS_MPI_CHAR, io_proc, (cur_proc * 2),
                  PS_MPI_COMM_WORLD, &(pms));
    }
  }
...
long load_int(FILE *fp, int *p_data, int cur_proc, int num_one_group,
              int num_max_proc) {
  return fread_multi_proc(p_data, sizeof(int), 1, fp, cur_proc, num_one_group,
                          num_max_proc);
}
long save_int(FILE *fp, int *p_data, int cur_proc, int num_one_group,
              int num_max_proc) {
  return fwrite_multi_proc(p_data, sizeof(int), 1, fp, cur_proc, num_one_group,
                           num_max_proc);
}
long load_long(FILE *fp, long *p_data, int cur_proc, int num_one_group,
               int num_max_proc) {
  return fread_multi_proc(p_data, sizeof(long), 1, fp, cur_proc, num_one_group,
                          num_max_proc);
}
long save_long(FILE *fp, long *p_data, int cur_proc, int num_one_group,
               int num_max_proc) {
  return fwrite_multi_proc(p_data, sizeof(long), 1, fp, cur_proc, num_one_group,
                           num_max_proc);
}
long load_float(FILE *fp, float *p_data, int cur_proc, int num_one_group,
                int num_max_proc) {
  return fread_multi_proc(p_data, sizeof(float), 1, fp, cur_proc, num_one_group,
                          num_max_proc);
}
long save_float(FILE *fp, float *p_data, int cur_proc, int num_one_group,
                int num_max_proc) {
  return fwrite_multi_proc(p_data, sizeof(float), 1, fp, cur_proc,
                           num_one_group, num_max_proc);
}
long load_double(FILE *fp, double *p_data, int cur_proc, int num_one_group,
                 int num_max_proc) {
  return fread_multi_proc(p_data, sizeof(double), 1, fp, cur_proc,
                          num_one_group, num_max_proc);
}
long save_double(FILE *fp, double *p_data, int cur_proc, int num_one_group,
                 int num_max_proc) {
  return fwrite_multi_proc(p_data, sizeof(double), 1, fp, cur_proc,
                           num_one_group, num_max_proc);
}
long load_Particle_in_Cell_MPI(FILE *fp, Particle_in_Cell_MPI *datas,
                               int cur_proc, int num_one_group,
                               int num_max_proc) {
  long num_saved;

  if (&((datas)->MPI_fieldE)) {
    (num_saved =
         (num_saved + load_Field3D_MPI_ALL(fp, &((datas)->MPI_fieldE), cur_proc,
                                           num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldB)) {
    (num_saved =
         (num_saved + load_Field3D_MPI(fp, &((datas)->MPI_fieldB), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_FoutJ)) {
    (num_saved =
         (num_saved + load_Field3D_MPI(fp, &((datas)->MPI_FoutJ), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldEtmp)) {
    (num_saved =
         (num_saved + load_Field3D_MPI(fp, &((datas)->MPI_fieldEtmp), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldE_ext)) {
    (num_saved =
         (num_saved + load_Field3D_MPI(fp, &((datas)->MPI_fieldE_ext), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldB_ext)) {
    (num_saved =
         (num_saved + load_Field3D_MPI(fp, &((datas)->MPI_fieldB_ext), cur_proc,
                                       num_one_group, num_max_proc)));
...
long save_Particle_in_Cell_MPI(FILE *fp, Particle_in_Cell_MPI *datas,
  ...
  if (&((datas)->MPI_fieldE)) {
    (num_saved =
         (num_saved + save_Field3D_MPI_ALL(fp, &((datas)->MPI_fieldE), cur_proc,
                                           num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldB)) {
    (num_saved =
         (num_saved + save_Field3D_MPI(fp, &((datas)->MPI_fieldB), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_FoutJ)) {
    (num_saved =
         (num_saved + save_Field3D_MPI(fp, &((datas)->MPI_FoutJ), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldEtmp)) {
    (num_saved =
         (num_saved + save_Field3D_MPI(fp, &((datas)->MPI_fieldEtmp), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldE_ext)) {
    (num_saved =
         (num_saved + save_Field3D_MPI(fp, &((datas)->MPI_fieldE_ext), cur_proc,
                                       num_one_group, num_max_proc)));
  ...
  if (&((datas)->MPI_fieldB_ext)) {
    (num_saved =
         (num_saved + save_Field3D_MPI(fp, &((datas)->MPI_fieldB_ext), cur_proc,
                                       num_one_group, num_max_proc)));
...
long load_Field3D_MPI_ALL(FILE *fp, Field3D_MPI_ALL *datas, int cur_proc,
                          int num_one_group, int num_max_proc) {
  long num_saved;

  {
    ...
    for ((tmpi2 = 0); (tmpi2 < (datas)->num_runtime); (tmpi2 = (tmpi2 + 1))) {
      (num_saved =
           (num_saved + load_Field3D_Seq(fp, ((datas)->data + tmpi2), cur_proc,
                                         num_one_group, num_max_proc)));
    }
  }
  {
    ...
    for ((tmpi2 = 0); (tmpi2 < ((datas)->num_runtime * (datas)->num_spec));
         (tmpi2 = (tmpi2 + 1))) {
      (num_saved = (num_saved + load_One_Particle_Collection(
                                    fp, ((datas)->particles + tmpi2), cur_proc,
                                    num_one_group, num_max_proc)));
    }
  }
  return num_saved;
}
long save_Field3D_MPI_ALL(FILE *fp, Field3D_MPI_ALL *datas, int cur_proc,
                          int num_one_group, int num_max_proc) {
  long num_saved;

  {
    ...
    for ((tmpi2 = 0); (tmpi2 < (datas)->num_runtime); (tmpi2 = (tmpi2 + 1))) {
      (num_saved =
           (num_saved + save_Field3D_Seq(fp, ((datas)->data + tmpi2), cur_proc,
                                         num_one_group, num_max_proc)));
    }
  }
  {
    ...
    for ((tmpi2 = 0); (tmpi2 < ((datas)->num_runtime * (datas)->num_spec));
         (tmpi2 = (tmpi2 + 1))) {
      (num_saved = (num_saved + save_One_Particle_Collection(
                                    fp, ((datas)->particles + tmpi2), cur_proc,
                                    num_one_group, num_max_proc)));
    }
  }
...
long load_Field3D_MPI(FILE *fp, Field3D_MPI *datas, int cur_proc,
                      int num_one_group, int num_max_proc) {
  long num_saved;
...
long save_Field3D_MPI(FILE *fp, Field3D_MPI *datas, int cur_proc,
                      int num_one_group, int num_max_proc) {
  long num_saved;
...
long load_Field3D_Seq(FILE *fp, Field3D_Seq *datas, int cur_proc,
                      int num_one_group, int num_max_proc) {
  long num_saved;

  {
    ...
    if ((0 == CD_type)) {
      ...
      if ((1 == CD_type)) {
        ...
        if ((2 == CD_type)) {
          cuda_pscmc_get_h_data((datas)->main_data, &(tmp_array));
    ...
    (num_saved =
         (num_saved +
          fread_multi_proc(
              tmp_array,
              (((0 == CD_type))
                   ? (c_pscmc_mem_get_num((datas)->main_data))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_mem_get_num((datas)->main_data))
                           : ((((2 == CD_type)) ? (cuda_pscmc_mem_get_num(
                                                      (datas)->main_data))
                                                : (0)))))),
              sizeof(double), fp, cur_proc, num_one_group, num_max_proc)));
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((datas)->main_data);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((datas)->main_data);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_h2d((datas)->main_data);
...
long save_Field3D_Seq(FILE *fp, Field3D_Seq *datas, int cur_proc,
                      int num_one_group, int num_max_proc) {
  long num_saved;

  {
    ...
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_d2h((datas)->main_data);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_d2h((datas)->main_data);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_d2h((datas)->main_data);
...

  {
    ...
    if ((0 == CD_type)) {
      ...
      if ((1 == CD_type)) {
        ...
        if ((2 == CD_type)) {
          cuda_pscmc_get_h_data((datas)->inoutput, &(tmp_array));
    ...
    (num_saved =
         (num_saved +
          fread_multi_proc(
              tmp_array,
              (((0 == CD_type))
                   ? (c_pscmc_mem_get_num((datas)->inoutput))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_mem_get_num((datas)->inoutput))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_mem_get_num((datas)->inoutput))
                                   : (0)))))),
              sizeof(double), fp, cur_proc, num_one_group, num_max_proc)));
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((datas)->inoutput);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((datas)->inoutput);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_h2d((datas)->inoutput);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      ...
      if ((1 == CD_type)) {
        ...
        if ((2 == CD_type)) {
          cuda_pscmc_get_h_data((datas)->xyzw, &(tmp_array));
    ...
    (num_saved =
         (num_saved +
          fread_multi_proc(
              tmp_array,
              (((0 == CD_type))
                   ? (c_pscmc_mem_get_num((datas)->xyzw))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_mem_get_num((datas)->xyzw))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_mem_get_num((datas)->xyzw))
                                   : (0)))))),
              sizeof(int), fp, cur_proc, num_one_group, num_max_proc)));
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((datas)->xyzw);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((datas)->xyzw);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_h2d((datas)->xyzw);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      ...
      if ((1 == CD_type)) {
        ...
        if ((2 == CD_type)) {
          cuda_pscmc_get_h_data((datas)->cu_cache, &(tmp_array));
    ...
    (num_saved =
         (num_saved +
          fread_multi_proc(
              tmp_array,
              (((0 == CD_type))
                   ? (c_pscmc_mem_get_num((datas)->cu_cache))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_mem_get_num((datas)->cu_cache))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_mem_get_num((datas)->cu_cache))
                                   : (0)))))),
              sizeof(double), fp, cur_proc, num_one_group, num_max_proc)));
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((datas)->cu_cache);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((datas)->cu_cache);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_h2d((datas)->cu_cache);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      ...
      if ((1 == CD_type)) {
        ...
        if ((2 == CD_type)) {
          cuda_pscmc_get_h_data((datas)->cu_xyzw, &(tmp_array));
    ...
    (num_saved =
         (num_saved +
          fread_multi_proc(
              tmp_array,
              (((0 == CD_type))
                   ? (c_pscmc_mem_get_num((datas)->cu_xyzw))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_mem_get_num((datas)->cu_xyzw))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_mem_get_num((datas)->cu_xyzw))
                                   : (0)))))),
              sizeof(int), fp, cur_proc, num_one_group, num_max_proc)));
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((datas)->cu_xyzw);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((datas)->cu_xyzw);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_h2d((datas)->cu_xyzw);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      ...
      if ((1 == CD_type)) {
        ...
        if ((2 == CD_type)) {
          cuda_pscmc_get_h_data((datas)->adjoint_vec_pids, &(tmp_array));
    ...
    (num_saved =
         (num_saved +
          fread_multi_proc(
              tmp_array,
              (((0 == CD_type))
                   ? (c_pscmc_mem_get_num((datas)->adjoint_vec_pids))
                   : ((((1 == CD_type))
                           ? (openmp_pscmc_mem_get_num(
                                 (datas)->adjoint_vec_pids))
                           : ((((2 == CD_type))
                                   ? (cuda_pscmc_mem_get_num(
                                         (datas)->adjoint_vec_pids))
                                   : (0)))))),
              sizeof(long), fp, cur_proc, num_one_group, num_max_proc)));
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((datas)->adjoint_vec_pids);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((datas)->adjoint_vec_pids);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_h2d((datas)->adjoint_vec_pids);
...
long save_One_Particle_Collection(FILE *fp, One_Particle_Collection *datas,
...

  {
    ...
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_d2h((datas)->inoutput);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_d2h((datas)->inoutput);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_d2h((datas)->inoutput);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_d2h((datas)->xyzw);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_d2h((datas)->xyzw);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_d2h((datas)->xyzw);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_d2h((datas)->cu_cache);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_d2h((datas)->cu_cache);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_d2h((datas)->cu_cache);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_d2h((datas)->cu_xyzw);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_d2h((datas)->cu_xyzw);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_d2h((datas)->cu_xyzw);
  ...
  }
  {
    ...
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_d2h((datas)->adjoint_vec_pids);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_d2h((datas)->adjoint_vec_pids);

      } else {
        if ((2 == CD_type)) {
          cuda_pscmc_mem_sync_d2h((datas)->adjoint_vec_pids);
...
int save_state_from_fp(FILE *fp, int cur_proc, int num_one_group,
                       int num_max_proc) {
...
int load_state_from_fp(FILE *fp, int cur_proc, int num_one_group,
                       int num_max_proc) {
  {
    long tmpvar;
...
FILE *load_identity_state_and_alloc_file_pointer(int mpi_rank,
                                                 int num_one_group) {
  char file_name[100];
  int write_rank = (mpi_rank / num_one_group);
