...
int merge_ovlp_mpi_field(Field3D_MPI *pthis) {
  ...
  for (i = 0; i < num_data; i++) {
    ...
    //====================Field3D_Seq
    ((v_offset)[i] = 0);
    ((all_sync_len)[i] = (numvec * (num_ele * ((xblock * (yblock * zblock)) -
                                               (xlen * (ylen * zlen))))));
  }
  int fieldid;
  ...
  for (fieldid = 0; (fieldid < NUM_SYNC_LAYER); fieldid++) {
    ...
    for (i = 0; i < num_data; i++) {
      ...
      if ((0 == CD_type)) {
        c_pscmc_get_h_data((sync_layer_pscmc)[0], &(sync_mem_host));

      } else {
        if ((1 == CD_type)) {
          openmp_pscmc_get_h_data((sync_layer_pscmc)[0], &(sync_mem_host));
      ...
      if ((fieldid == 0)) {
        if ((0 == CD_type)) {
          c_pscmc_mem_sync_d2h((sync_layer_pscmc)[0]);

        } else {
          if ((1 == CD_type)) {
            openmp_pscmc_mem_sync_d2h((sync_layer_pscmc)[0]);
      ...
      for (tid = 0; (tid < numvec); tid++) {
        ...
        int REMOTE_PROC_ID =
            ((adj_processes)[((tid * NUM_SYNC_LAYER) + fieldid)] /
             (pthis)->num_runtime);

        {
          if (((adj_processes)[((tid * NUM_SYNC_LAYER) + fieldid)] ==
               (adj_processes)[((tid * NUM_SYNC_LAYER) +
                                (NUM_SYNC_LAYER / 2))])) {
            continue;

          } else {
            PS_MPI_Isend(
                (t0 + (tid * sllen)), sllen, PS_MPI_DOUBLE, REMOTE_PROC_ID,
                ((NUM_SYNC_LAYER *
                  (adj_ids)[((tid * NUM_SYNC_LAYER) + fieldid)]) +
                 fieldid),
                comm, ((rqst)[i] + ((tid * NUM_SYNC_LAYER) + fieldid)));
          }
        }
      }
    }
    for (i = 0; i < num_data; i++) {
      ...
      if ((0 == CD_type)) {
        c_pscmc_get_h_data((swap_layer_pscmc)[0], &(swap_mem_host));

      } else {
        if ((1 == CD_type)) {
          openmp_pscmc_get_h_data((swap_layer_pscmc)[0], &(swap_mem_host));
      ...
      for (tid = 0; (tid < numvec); tid++) {
        ...
        {
          long adj_proc_id =
              (adj_processes)[((tid * NUM_SYNC_LAYER) + fieldid1)];

          int REMOTE_PROC_ID = (adj_proc_id / (pthis)->num_runtime);

          if ((adj_proc_id == (adj_processes)[((tid * NUM_SYNC_LAYER) +
                                               (NUM_SYNC_LAYER / 2))])) {
            long t0id = (adj_local_tid)[((tid * NUM_SYNC_LAYER) + fieldid1)];

            memcpy((t1 + (tid * sllen)), (t0 + (t0id * sllen)),
                   (sizeof(double) * sllen));

          } else {
            PS_MPI_Recv(
                (t1 + (tid * sllen)), sllen, PS_MPI_DOUBLE, REMOTE_PROC_ID,
                ((NUM_SYNC_LAYER *
                  (adj_ids)[((tid * NUM_SYNC_LAYER) + (NUM_SYNC_LAYER / 2))]) +
  ...
  for (i = 0; i < num_data; i++) {
    ...
    //====================Field3D_Seq
    if ((0 == CD_type)) {
      c_pscmc_mem_sync_h2d((swap_layer_pscmc)[0]);

    } else {
      if ((1 == CD_type)) {
        openmp_pscmc_mem_sync_h2d((swap_layer_pscmc)[0]);
    ...
    for (fieldid = 0; (fieldid < NUM_SYNC_LAYER); fieldid++) {
      ...
      if ((0 == CD_type)) {
        ...
        if ((1 == CD_type)) {
          ...
          if ((2 == CD_type)) {
            cuda_pscmc_mem_sync_d2h((sync_layer_pscmc)[fieldid]);
      ...
      for (tid = 0; (tid < numvec); tid++) {
        ...
        if ((0 == CD_type)) {
          ...
          if ((1 == CD_type)) {
            ...
            if ((2 == CD_type)) {
              cuda_pscmc_get_h_data((sync_layer_pscmc)[fieldid], &(t0));
    ...
    for (fieldid = 0; (fieldid < NUM_SYNC_LAYER); fieldid++) {
      ...
      for (tid = 0; (tid < numvec); tid++) {
        ...
        if ((0 == CD_type)) {
          ...
          if ((1 == CD_type)) {
            ...
            if ((2 == CD_type)) {
              cuda_pscmc_get_h_data((swap_layer_pscmc)[fieldid], &(t1));
        ...
        if ((0 == CD_type)) {
          c_pscmc_get_h_data(
              (sync_layer_pscmc)[((NUM_SYNC_LAYER - 1) - fieldid)], &(t0));
      ...
      if ((0 == CD_type)) {
        ...
        if ((1 == CD_type)) {
          ...
          if ((2 == CD_type)) {
            cuda_pscmc_mem_sync_h2d((swap_layer_pscmc)[fieldid]);
