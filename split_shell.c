...
double wclk_now();
int split_1st_all_passes(Particle_in_Cell_MPI *pthis, double dt0) {
...
int pass_GeqB(Particle_in_Cell_MPI *pthis, double deltat) {
  ...
  void **dm8x8_kernels = (pMPI_fieldE->data)->dm8x8_kernels;

  void **geo_yeefdtd_kernels = (pMPI_fieldE->data)->geo_yeefdtd_kernels;

  void **geo_yeefdtd_rect_kernels =
      (pMPI_fieldE->data)->geo_yeefdtd_rect_kernels;

  void **yeefdtd_stencil_kernels = (pMPI_fieldE->data)->yeefdtd_stencil_kernels;
  ...
  void *blas_yiszero_synced_kernel =
      (pMPI_fieldE->data)->blas_yiszero_synced_kernel;

  void *blas_mulxy_numele3_kernel =
      (pMPI_fieldE->data)->blas_mulxy_numele3_kernel;

  void *blas_yiszero_kernel = (pMPI_fieldE->data)->blas_yiszero_kernel;

  void *blas_yiszero_full_block_kernel =
      (pMPI_fieldE->data)->blas_yiszero_full_block_kernel;

  void *blas_yisconst_kernel = (pMPI_fieldE->data)->blas_yisconst_kernel;

  void *blas_yisconst_full_block_kernel =
      (pMPI_fieldE->data)->blas_yisconst_full_block_kernel;

  void *blas_get_ITG_Potential_kernel =
      (pMPI_fieldE->data)->blas_get_ITG_Potential_kernel;

  void *blas_get_ITG_Potential_full_block_kernel =
      (pMPI_fieldE->data)->blas_get_ITG_Potential_full_block_kernel;

  void *blas_invy_kernel = (pMPI_fieldE->data)->blas_invy_kernel;

  void *blas_invy_full_block_kernel =
      (pMPI_fieldE->data)->blas_invy_full_block_kernel;

  void *blas_axpby_kernel = (pMPI_fieldE->data)->blas_axpby_kernel;

  void *blas_axpby_full_block_kernel =
      (pMPI_fieldE->data)->blas_axpby_full_block_kernel;

  void *blas_axpy_kernel = (pMPI_fieldE->data)->blas_axpy_kernel;

  void *blas_axpy_full_block_kernel =
      (pMPI_fieldE->data)->blas_axpy_full_block_kernel;

  void *blas_yisax_kernel = (pMPI_fieldE->data)->blas_yisax_kernel;

  void *blas_yisax_full_block_kernel =
      (pMPI_fieldE->data)->blas_yisax_full_block_kernel;

  void *blas_mulxy_kernel = (pMPI_fieldE->data)->blas_mulxy_kernel;

  void *blas_mulxy_full_block_kernel =
      (pMPI_fieldE->data)->blas_mulxy_full_block_kernel;

  void *blas_findmax_kernel = (pMPI_fieldE->data)->blas_findmax_kernel;

  void *blas_findmax_full_block_kernel =
      (pMPI_fieldE->data)->blas_findmax_full_block_kernel;

  void *blas_dot_kernel = (pMPI_fieldE->data)->blas_dot_kernel;

  void *blas_dot_full_block_kernel =
      (pMPI_fieldE->data)->blas_dot_full_block_kernel;

  void *blas_sum_kernel = (pMPI_fieldE->data)->blas_sum_kernel;

  void *blas_sum_full_block_kernel =
      (pMPI_fieldE->data)->blas_sum_full_block_kernel;

  void *blas_yisax_shrink_kernel =
      (pMPI_fieldE->data)->blas_yisax_shrink_kernel;

  void *blas_yisax_enlarge_kernel =
      (pMPI_fieldE->data)->blas_yisax_enlarge_kernel;

  void *blas_axpy_shrink_kernel = (pMPI_fieldE->data)->blas_axpy_shrink_kernel;

  void *blas_axpy_enlarge_kernel =
      (pMPI_fieldE->data)->blas_axpy_enlarge_kernel;

  void *blas_axpby_shrink_kernel =
      (pMPI_fieldE->data)->blas_axpby_shrink_kernel;

  void *blas_axpby_enlarge_kernel =
      (pMPI_fieldE->data)->blas_axpby_enlarge_kernel;

  //====================Field3D_Seq
  if (use_pml_abc_dir) {
    sync_ovlp_mpi_field(&(MPI_fieldPMLB));
    MPI_PML_FDTD_CURL_BWD(pMPI_fieldE, pMPI_fieldB, &(MPI_fieldPMLE),
                          &(MPI_fieldPMLB), deltat, 0, 0, delta_x, delta_y,
                          delta_z, use_pml_abc_dir, use_pml_level, 3,
                          use_pml_sigma_max, allxmax, allymax, allzmax);

  } else {
    MPI_RECT_YEE_CURL_L(pMPI_fieldE, pMPI_fieldB, delta_z, delta_y, delta_x,
                        deltat);
  }
...
int ITG_split_2nd_all_passes_enecons(Particle_in_Cell_MPI *pthis,
                                     Field3D_MPI *pext_rho_dist, double dt0,
                                     double moqe, double ne0, int use_gc) {
  ...
  blas_get_ITG_Potential_Field3D_MPI(pMPI_FoutJ, pMPI_FoutJ, &(MPI_fieldB1),
                                     pMPI_fieldE, moqe);
  sync_ovlp_mpi_field(pMPI_FoutJ);
  MPI_Yee_FDTD_Grad_FWD(pMPI_fieldE, pMPI_FoutJ, 0);
  if (USE_INIT_EXT_EB) {
    blas_axpy_Field3D_MPI(&(MPI_fieldE), &(MPI_fieldE), 1.00000000000000000e+00,
                          &(MPI_fieldE_ext));
  ...
  if (USE_INIT_EXT_EB) {
    blas_axpy_Field3D_MPI(&(MPI_fieldE), &(MPI_fieldE),
                          -1.00000000000000000e+00, &(MPI_fieldE_ext));

  } else {
    0;
  }

  blas_yisax_Field3D_MPI(pMPI_fieldE, pMPI_fieldE, 1.00000000000000000e+00,
                         pMPI_fieldB);
  Field3D_MPI *pMPI_fieldEtmp = &(MPI_fieldEtmp);

  blas_yisax_Field3D_MPI(pMPI_fieldEtmp, pMPI_fieldEtmp,
                         1.00000000000000000e+00, pMPI_FoutJ);
  blas_yisconst_Field3D_MPI(pMPI_fieldB, pMPI_fieldB, 0);
  split_pass_xyz_zyx_mpi_shell(pthis, dt0, 0, 0);
  blas_yisax_Field3D_MPI(pMPI_FoutJ, pMPI_FoutJ, 1.00000000000000000e+00,
                         pMPI_fieldEtmp);
  return 0;
}
int ITG_split_2nd_all_passes(Particle_in_Cell_MPI *pthis,
                             Field3D_MPI *pext_rho_dist, double dt0,
                             double moqe, double ne0, int use_gc) {
  ...
  MPI_Yee_FDTD_Grad_FWD(pMPI_fieldE, pMPI_FoutJ, 0);
  if ((0 && (ct == 20))) {
    fprintf(stderr, "mass=%e, chg=%e\n", ((pthis->MPI_fieldE).particles)->Mass,
            ((pthis->MPI_fieldE).particles)->Charge);
  ...
  if (use_gc) {
    if (USE_INIT_EXT_EB) {
      blas_axpy_Field3D_MPI(&(MPI_fieldB), &(MPI_fieldB),
                            1.00000000000000000e+00, &(MPI_fieldB_ext));
    ...
    MPI_ngeo_gc(pthis, pMPI_fieldE, pMPI_fieldB, pMPI_fieldB, pMPI_FoutJ,
                &(oms), &(ochg), dt0, 1, 1, 1, 1, 1, 1);
...
int split_multi_step_passes_test(Particle_in_Cell_MPI *pthis, double dt0,
                                 int N_electron, int N_maxwell, double *pmass,
                                 double *pchg) {
  ...
  }
  {
    ...
    blas_yiszero_synced_Field3D_MPI(pMPI_FoutJ, pMPI_FoutJ);
    MPI_split_pass_E_particle(pthis, pMPI_fieldEtmp, pMPI_fieldB, pMPI_FoutJ,
                              pMPI_FoutJ, pmass, pchg, dt0);
    {
      long i;

      for ((i = 0); (i < N_electron); (i = (i + 1))) {
        MPI_split_pass_x(pthis, pMPI_fieldEtmp, pMPI_fieldB, pMPI_FoutJ,
                         pMPI_FoutJ, pmass, pchg, dt);
        call_particle_sort_mpi(pthis, 0, 0);
        MPI_split_pass_y(pthis, pMPI_fieldEtmp, pMPI_fieldB, pMPI_FoutJ,
                         pMPI_FoutJ, pmass, pchg, dt);
        call_particle_sort_mpi(pthis, 1, 0);
        MPI_split_pass_z(pthis, pMPI_fieldEtmp, pMPI_fieldB, pMPI_FoutJ,
                         pMPI_FoutJ, pmass, pchg, dt);
        call_particle_sort_mpi(pthis, 2, 0);
      }
    }
...
int split_2nd_all_passes_multi_step(Particle_in_Cell_MPI *pthis, double dt0,
                                    int use_ext_G_E, int N_maxwell,
                                    int N_electron, double *pmass,
                                    double *pchg) {
      ...
      for ((i = 0); (i < N_maxwell); (i = (i + 1))) {
        ...
        if (use_pml_abc_dir) {
          ...
          void **yeefdtd_stencil_kernels =
              (pMPI_fieldE->data)->yeefdtd_stencil_kernels;
          ...
          void *blas_yisconst_kernel =
              (pMPI_fieldE->data)->blas_yisconst_kernel;
          ...
          void *blas_axpy_shrink_kernel =
              (pMPI_fieldE->data)->blas_axpy_shrink_kernel;
          ...
          MPI_PML_FDTD_CURL_FWD(
              pMPI_fieldB, pMPI_fieldE, &(MPI_fieldPMLB), &(MPI_fieldPMLE),
              dt_Bmid, 0, 0, delta_x, delta_y, delta_z, use_pml_abc_dir,
              use_pml_level, 3, use_pml_sigma_max, allxmax, allymax, allzmax);

        } else {
          MPI_Yee_FDTD_Curl_E(pMPI_fieldB, pMPI_fieldE, dt_Bmid);
        }

        blas_axpy_Field3D_MPI(pMPI_fieldEtmp2, pMPI_fieldEtmp2,
                              (dt_Bmid / newdt), pMPI_fieldE);
        if (USE_PROFILE) {
          PS_MPI_Barrier(PS_MPI_COMM_WORLD);
          double cur_time = wclk_now();
  ...
  if ((USE_INIT_EXT_EB && (!use_ext_G_E))) {
    blas_axpy_Field3D_MPI(pMPI_fieldEtmp2, pMPI_fieldEtmp2, 1,
                          &(MPI_fieldE_ext));
  ...
  if (use_vlo) {
    MPI_split_pass_E_particle_vlo(pthis, pMPI_fieldEtmp2, pMPI_fieldB,
                                  pMPI_FoutJ, pMPI_FoutJ, pmass, pchg,
                                  (dt0 * 2));

  } else {
    MPI_split_pass_E_particle(pthis, pMPI_fieldEtmp2, pMPI_fieldB, pMPI_FoutJ,
                              pMPI_FoutJ, pmass, pchg, (dt0 * 2));
  }

  if ((USE_INIT_EXT_EB && use_ext_G_E)) {
    blas_yisax_Field3D_MPI(&(MPI_fieldEtmp), &(MPI_fieldEtmp),
                           1.00000000000000000e+00, &(MPI_fieldE));
    blas_yisax_Field3D_MPI(&(MPI_fieldE), &(MPI_fieldE),
                           1.00000000000000000e+00, &(MPI_fieldE_ext));
    ...
    blas_yisax_Field3D_MPI(&(MPI_fieldE), &(MPI_fieldE),
                           1.00000000000000000e+00, &(MPI_fieldEtmp));
...
int split_multi_substep_passes(Particle_in_Cell_MPI *pthis, double dt0,
                               int N_electron) {
...
int split_2nd_all_passes_Eout(Particle_in_Cell_MPI *pthis, double dt0,
                              int use_ext_G_E) {
  ...
  if (use_pml_abc_dir) {
    ...
    //====================Field3D_Seq
    sync_ovlp_mpi_field(&(MPI_fieldPMLE));
    MPI_PML_FDTD_CURL_FWD(pMPI_fieldB, pMPI_fieldE, &(MPI_fieldPMLB),
                          &(MPI_fieldPMLE), (dt0 * 2), 0, 0, delta_x, delta_y,
                          delta_z, use_pml_abc_dir, use_pml_level, 3,
                          use_pml_sigma_max, allxmax, allymax, allzmax);
...
int split_2nd_all_passes(Particle_in_Cell_MPI *pthis, double dt0,
                         int use_ext_G_E, int num_multi_substep) {
